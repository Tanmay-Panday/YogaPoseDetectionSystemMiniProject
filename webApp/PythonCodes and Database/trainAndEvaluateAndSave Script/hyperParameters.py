...
def hyperparameter_tuning(X_train, y_train, param_grid, model, classifier_name):
    grid_search = GridSearchCV(model, param_grid, cv=cv, scoring='accuracy')
    grid_search.fit(X_train, y_train)

    print(f"Best parameters for {classifier_name}: {grid_search.best_params_}")
    print(f"Best cross-validation accuracy for {classifier_name}: {grid_search.best_score_}")
...
def save_metrics_and_plot_to_pdf(classifier_name, metrics, confusion_matrix_plot):
    ...
    pdf_canvas.drawString(50, 730,
                          f"  Accuracy: {metrics['Accuracy']} ,   Precision: {metrics['Precision']} ,   Recall: {metrics['Recall']} ,   F1 Score: {metrics['F1 Score']}")
    ...
    pdf_canvas.drawInlineImage(confusion_matrix_image, x_position, y_position, width=target_width,
...
def train_and_evaluate_classifier(X_train, X_test, y_train, y_test, clf, param_grid):
    # Hyperparameter tuning
    tuned_model = hyperparameter_tuning(X_train, y_train, param_grid, clf, clf.__class__.__name__)

    # Train the model using the tuned hyperparameters
    model = make_pipeline(StandardScaler(), tuned_model)
    ...
    return model, accuracy, classifier_name
...
def plot_roc_curve_for_all_models(final_models, X_test, y_test, class_labels):
    ...
    for model, clf_name, _ in final_models:
        ...
        fpr, tpr, _ = roc_curve(lb.ravel(), y_score.ravel())  # no need for threshold
        roc_auc = auc(fpr, tpr)
...
def trainAndSavePdfsAndModel():
    classifiers = {
        'Logistic Regression': (LogisticRegression(max_iter=1000), param_grid_lr),  # regularization
        'Random Forest': (RandomForestClassifier(), param_grid_rf),  # hyperparameter tuning reducing decision trees
        'Gradient Boosting': (GradientBoostingClassifier(), param_grid_gb)  # hyperparameter tuning reducing decision trees
    }
    best_model = None
    best_accuracy = 0.0
    ...
    for clf_name, (clf, param_grid) in classifiers.items():
        model, accuracy, classifier_name = train_and_evaluate_classifier(X_train, X_test, y_train, y_test, clf,
                                                                          param_grid)
        final_Models.append([model, classifier_name, param_grid])
